...
- Bundles your Electron main and preload scripts from `rulect/entry.ts` or `rulect/entry.js` and `rulect/preload.ts` or `rulect/preload.js`.
...
- Expects a `rulect` directory with `entry` and `preload` source files.
